{
  description = "Slippi overlay";

  inputs = {
    nixpkgs.url = "github:NixOS/nixpkgs/nixos-unstable";
    slippiDesktopApp = {
      url = "github:project-slippi/slippi-desktop-app";
      flake = false;
    };
  };

  outputs = { self, nixpkgs, slippiDesktopApp }:
    let
      systems = [ "x86_64-linux" ];
      forAllSystems = f: nixpkgs.lib.genAttrs systems (system: f system);

      slippi = system: playbackSlippi:
        with import nixpkgs { inherit system; };
        stdenv.mkDerivation rec {
          pname = "slippi-ishiiruka";
          version = "2.2.1";
          name = "${pname}-${version}-${
              if playbackSlippi then "playback" else "netplay"
            }";
          src = builtins.path {
            path = ./.;
            inherit name;
    ...
    in {
      packages = forAllSystems (system: {
        netplay = slippi system false;
        playback = slippi system true;
      });
    };
}
